...
import { loginWithCopilotDeviceFlow } from "../services/copilot/provider.js";
import {
...
export function Onboarding({ onDone }: Props): React.ReactNode {
  ...
  steps.push({
    id: "provider",
    component: <ProviderOnboardingStep onDone={goToNextStep} />,
  });
  steps.push({
    id: "security",
  ...
  if (shouldOfferTerminalSetup()) {
    steps.push({
      ...
      component: (
        <Box flexDirection="column" gap={1} paddingLeft={1}>
          <Text bold>Use Obsidian Code&apos;s terminal setup?</Text>
...
function ProviderOnboardingStep({
  ...
  const [view, setView] = useState<ProviderOnboardingView>("providers");
  const [activeSection, setActiveSection] = useState<"connected" | "available">(
    "connected",
  );
  ...
  async function handleProviderSelection(
    ...
    try {
      ...
      setError(
        `Failed to use ${getAPIProviderLabel(provider)}: ${
  ...
  const availableOptions = available.map((item) => ({
    value: getCLIProviderName(item.provider),
    label: item.label,
  ...
  return (
    <Box flexDirection="column" gap={1} paddingLeft={1}>
      ...
      <Text dimColor>
        Select a connected provider to continue, or choose an available provider
        to connect it now.
      </Text>
      ...
      {view === "providers" ? (
        <Box flexDirection="column" gap={1}>
          ...
          {connected.length > 0 ? (
            <Box flexDirection="column" marginTop={1} gap={1}>
              ...
              <Select
                ...
                isDisabled={activeSection !== "connected"}
                onFocus={() => {
                  setActiveSection("connected");
                }}
                onChange={(value) => {
                  ...
                  if (!provider) return;
                  void handleProviderSelection(provider);
                }}
                onDownFromLastItem={
                  available.length > 0
                    ? () => {
          ...
          {available.length > 0 ? (
            <Box flexDirection="column" marginTop={1} gap={1}>
              ...
              <Select
                ...
                isDisabled={activeSection !== "available"}
                defaultFocusValue={
                  available[0]
                ...
                visibleOptionCount={availableOptions.length}
              />
            </Box>
          ) : null}
          ...
          {connected.length > 0 && available.length > 0 ? (
            <Text dimColor>
              ...
              Providers, or ↑ on the first available provider to return.
            </Text>
          ) : null}
        </Box>
        ...
        <Box flexDirection="column" gap={1}>
          ...
          <ConsoleOAuthFlow
            startingMessage="Sign in with your Anthropic account to continue."
            onDone={async () => {
              try {
        ...
        <Box flexDirection="column" gap={1}>
          ...
          <Text dimColor>Paste an OpenRouter API key.</Text>
            ...
            <TextInput
              ...
              onSubmit={async (value) => {
                ...
                try {
                  ...
                  await activateProvider("openrouter");
                  onDone();
                } catch (error) {
                  setError(
                    `Failed to connect OpenRouter: ${
                      error instanceof Error ? error.message : String(error)
                    }`,
                  );
                  setView("providers");
          ...
          <Select
            options={[
              ...
              },
              {
                ...
                value: "enterprise",
                description: "Self-hosted or enterprise-managed GitHub",
              },
            ]}
