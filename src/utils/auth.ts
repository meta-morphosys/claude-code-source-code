...
export function getOpenRouterApiKeyWithSource(): {
  ...
  const key =
    process.env.OPENROUTER_API_KEY || getGlobalConfig().env.OPENROUTER_API_KEY;
  if (key) {
...
/** Check if using third-party services (Bedrock or Vertex or Foundry or OpenRouter or Copilot) */
...
export function getAccountInformation() {
  ...
  if (apiProvider === "copilot") {
    const accountInfo: UserAccountInfo = {};
    if (getCopilotToken()) {
      accountInfo.tokenSource = "GITHUB_COPILOT_ACCESS_TOKEN";
      const enterpriseUrl = getCopilotEnterpriseUrl();
      if (enterpriseUrl) {
        accountInfo.organization = enterpriseUrl;
      }
    }
    return accountInfo;
  }
