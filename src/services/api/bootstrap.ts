...
import {
  fetchCopilotModelOptions,
  getCopilotToken,
} from "src/services/copilot/provider.js";
import {
  fetchOpenRouterModelOptions,
  getSavedOpenRouterApiKey,
} from "src/services/openrouter/provider.js";
import {
  getAnthropicApiKey,
...
async function fetchBootstrapAPI(): Promise<BootstrapResponse | null> {
  ...
  const copilotModels = getCopilotToken()
    ? await fetchCopilotModelOptions().catch((error) => {
        logForDebugging(
          `[Bootstrap] Copilot model fetch failed: ${errorMessage(error)}`,
        );
        return [];
      })
    : [];
  const openRouterModels = getSavedOpenRouterApiKey()
    ? await fetchOpenRouterModelOptions().catch((error) => {
        logForDebugging(
          `[Bootstrap] OpenRouter model fetch failed: ${errorMessage(error)}`,
        );
        return [];
      })
    : [];
  ...
  if (getAPIProvider() !== "firstParty") {
    ...
    return {
      client_data: null,
      additional_model_options: [...copilotModels, ...openRouterModels],
    };
  }
  ...
  try {
    return await withOAuth401Retry(async () => {
      ...
      return {
        client_data: parsed.data.client_data ?? null,
        additional_model_options: [
          ...(parsed.data.additional_model_options ?? []),
          ...copilotModels,
          ...openRouterModels,
        ],
      };
...
export async function fetchBootstrapData(): Promise<void> {
  try {
    ...
    const additionalModelOptions = Array.from(
      new Map(
        (response.additional_model_options ?? []).map((option) => [
          `${option.provider ?? "unknown"}:${option.value}`,
          option,
        ]),
      ).values(),
    );
